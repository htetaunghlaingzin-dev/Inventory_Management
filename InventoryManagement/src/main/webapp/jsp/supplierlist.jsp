...
<body>
<%@ page import="java.util.*, Model.SupplierAccess, Model.SuppllierBean" %>
<%
    // Only fetch data if not already set by the servlet
    if (request.getAttribute("SupplierList") == null) {
        SupplierAccess ua = new SupplierAccess();
        List<SuppllierBean> users = ua.viewUser();
        request.setAttribute("SupplierList", users);
    }
%>
...
<ul>
<li><a href="supplierlist.jsp" class="active">Supplier List</a></li>
...
<c:forEach var="supplier" items="${SupplierList}">
    <tr>
        ...
        <td>${supplier.country}</td> <!-- email replaced with country -->
        <td>${supplier.city}</td>    <!-- country replaced with city -->
        <td>
            <a class="me-3" href="/InventoryManagement/SupplierServlet?cmd=edit&id=${supplier.sid}">
                <img src="assets/img/icons/edit.svg" alt="img">
            </a>
            <a class="me-3 confirm-text" href="/InventoryManagement/SupplierServlet?cmd=delete&id=${supplier.sid}">
